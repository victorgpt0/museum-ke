    ...
    public function update(Request $request, Goals $goal)
    {
        ...
        if ($goal->completed) {
            if ($request->expectsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'This goal has already been completed and cannot be modified.',
                ], 422);
            }
            return back()->withErrors([
                'error' => 'This goal has already been completed and cannot be modified.',
            ]);
        }

        // Validate incoming data
        $validator = Validator::make($request->all(), [
            'performance' => 'nullable|integer|min:1|max:10',
        ...
        try {
            // Update the goal and mark as completed
            $goal->update([
                'performance' => $request->performance,
            ...
            return redirect()->back()->with('success', 'Goal completed successfully!');
